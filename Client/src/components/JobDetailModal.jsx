                            ...
                            <div className="md:col-span-2 space-y-8">
                                <div>
                                    ...
                                    <p className="text-gray-600 leading-relaxed whitespace-pre-line">
                                        {job.jobDescription || job.description || job.fullDescription}
                                    </p>
                                </div>

                                {job.requirements && parseRequirements(job.requirements).length > 0 && (
                                    <div>
                                        <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                                        ...
                                        <ul className="space-y-3">
                                            {parseRequirements(job.requirements).map((req, i) => (
                                                <li key={i} className="flex gap-3 text-gray-700">
                                                    <div className="mt-1 w-5 h-5 rounded-full bg-blue-100 flex items-center justify-center shrink-0 text-blue-600">
                                ...
                                {job.responsibilities && parseRequirements(job.responsibilities).length > 0 && (
                                    <div>
                                        <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                                        ...
                                        <ul className="space-y-3">
                                            {parseRequirements(job.responsibilities).map((req, i) => (
                                                <li key={i} className="flex gap-3 text-gray-700">
                                                    <div className="mt-1 w-5 h-5 rounded-full bg-purple-100 flex items-center justify-center shrink-0 text-purple-600">
                                                        <CheckCircle2 className="w-3 h-3" />
                                                    </div>
                                                    <span className="text-sm md:text-base">{req}</span>
                                                </li>
                            ...
                            <div className="md:col-span-1">
                                <div className="bg-white p-6 rounded-xl border border-gray-200 sticky top-0">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {job.skills && parseSkills(job.skills).map((skill, i) => (
                                            <span key={i} className="bg-blue-50 text-blue-700 text-xs px-3 py-1.5 rounded-lg font-medium border border-blue-100">
                                                {skill}
                                            </span>
                                        ))}
                                    </div>
